...
export default function FocusTime() {
  ...
  const handleIncreaseFocus = () => {
    timerCtx.focusTime >= 20
      ? timerCtx.setFocusTime((prev) => prev)
      : timerCtx.setFocusTime((prev) => prev + 1);
  };

  const handleDecreaseFocus = () => {
    timerCtx.focusTime <= 1
      ? timerCtx.setFocusTime((prev) => prev)
      : timerCtx.setFocusTime((prev) => prev - 1);
  };
  ...
  return (
    <div className="w-4/5 h-10 py-6 px-4 rounded-full bg-blue-500 text-white font-bold flex justify-between items-center ">
      <span>Focus Time</span>
      <div className="flex justify-between items-center w-11">
        <input
          type="text"
          value={timerCtx.focusTime}
        ...
        <div className="flex flex-col space-y-1 justify-between items-center">
          <button onClick={handleIncreaseFocus}>
            <svg
          ...
          <button onClick={handleDecreaseFocus}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
              width="10px"
